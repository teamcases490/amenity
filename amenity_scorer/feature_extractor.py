...
import config
from utils import safe_divide, gini_coefficient
...
class FeatureExtractor:
    ...
    def _quality_features(self, pois: List[Dict]) -> Dict:
        ...
        return {
            "health_quality_ratio":    safe_divide(_count_types(config.PREMIUM_POIS['healthcare']), _count_types(config.BASIC_POIS['healthcare']) + 1),
            "education_quality_ratio": safe_divide(_count_types(config.PREMIUM_POIS['education']), _count_types(config.BASIC_POIS['education']) + 1),
            "retail_sophistication":   safe_divide(_count_types(config.PREMIUM_POIS['shopping']), _count_types(config.BASIC_POIS['shopping']) + 1),
            "transport_quality":       safe_divide(_count_types(config.PREMIUM_POIS['transport']), _count_types(config.BASIC_POIS['transport']) + 1),
        }
    ...
    def _economic_features(self, pois: List[Dict]) -> Dict:
        ...
        cons_density = _density(config.CATEGORIES['shopping'] + config.CATEGORIES['food'])
