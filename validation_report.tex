...
\text{Score} &= \min\left(100, \frac{G}{200} \times 100\right)
\end{align}

$w_i$ = importance weight; $d_i$ = distance in km (floor at 100m).
The raw gravity sum $G$ is normalised against a benchmark of 200 (e.g., 5 amenities at 200m). This linear scaling provides a clearer gradient than the previous log-based approach.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Validation Results (v2.1)}
% ─────────────────────────────────────────────────────────────────────────────
...
\begin{enumerate}
  \item \textbf{API dependency.} The system needs the OSM Overpass API. While v2.1 includes robust retries, extreme network issues or API downtime can still block fresh data fetching.
